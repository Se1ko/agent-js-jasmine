    ...
    async specStarted(spec) {
        ...
        return this.currentPromise = promise.then((codeRef) => {
            ...
            if(this.needToSlice == false) {
                this.setParentInfo({ tempId: stepObj.tempId, realID: stepObj.realId, startTime: this.itemStartTime, name: (spec.fullName.indexOf('Allure') == -1) ? "duplicate" : spec.description, type: "spec", turn: this.middleTurn});
            }
            else {
                //console.log("Splice")
                this.spliceParentInfo((this.middlePosition == -1) ? this.parentsInfo.length - 1 : this.middlePosition, { tempId: stepObj.tempId, realID: stepObj.realId, startTime: this.itemStartTime, name: spec.description, type: "spec", turn:this.middleTurn});
                this.needToSlice = false;
            }
    ...
    async specDoneAttach(spec, screenshot) {
       ...
       //("screenshot:" + screenshot)
        ...
        return this.currentPromise = promise.then((fileObj) => {
            ...
            let parent;
            if(middleAlgo == true && turn == this.middleTurn) {
            ...
            if(debugMode) {
                console.log(`${spec.description}: delete spec ${parent.name} by ${deleteMode}`);  
            }
