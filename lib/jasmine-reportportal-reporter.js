...
class ReportportalReporter {
    constructor(conf, onSetLaunchStatus) {
        ...
        this.suiteCount = 0;
        this.clientPromise;
        this.middleAlgorithm = false;
        this.middleParent;
        this.middlePosition = -1;
        let debug = conf.debugMode;
        this.middleTurn = 1;
    }
    ...
    getMapParentInfo(name) {
        return this.parentsInfo.find(el => el.name == name);
    }
    ...
    finishParentByPosition(position, type) {
        //console.log(`${this.getParentInfo().type} == ${type}`)
        ...
        if(this.getParentInfo().type == type) {
            ...
            this.parentsInfo.splice(position, 1);
        }
    }

    finishMapParent(name) {
        const items = this.parentsInfo.map(el => el.name);
        position = items.indexOf(name)
        this.parentsInfo.splice(positions, 1);
    }

    popParent() {
        return this.parentsInfo.pop();
    }
    ...
    */
    async suiteStarted(suite) {
       ...
       // console.log(this.getSuiteStatusBySuite(suite));
        ...
        return this.currentPromise = promise.then((codeRef) => {
            ...
            this.setParentInfo({ tempId: suiteObj.tempId, startTime: this.itemStartTime, name: suite.description, type: 'suite' });
    ...
    async specStarted(spec) {
        ...
        //Чтобы отключить логирование xdescribe спек
        if (spec.fullName.indexOf('x') == 0 || spec.description.indexOf('x') == 0 || spec.fullName == "") {
        ...
        return this.currentPromise = promise.then((codeRef) => {
            ...
            if(this.middleAlgorithm == true) {
                this.middlePosition++;
                this.needToSlice = true;
                //console.log(`Middle slice Position:${this.middlePosition}`)
            }

            let parent = this.getParentInfo();
            let slice = true;
            if(spec.fullName.indexOf('Spec') != -1) {
                let pLength = this.parentsInfo.length;
                for (let i = 0; i < pLength; i++) {
                    if(this.parentsInfo[i].type == "suite") {
                        if(i+1 < pLength && this.parentsInfo[i+1].type == "suite") {
                            console.log('Continue');
                            continue;
                        }
                        this.middleParent = parent = this.parentsInfo[i];
                        slice = false;
                        this.needToSlice = true;
                        this.middlePosition = i +1;
                        this.middleAlgorithm = true;
                        //console.log(`middle algorithm Parent:${parent.type} Position:${this.middlePosition}`)
                        break;
                    }
                }
            }
            if(parent.type == "spec" && slice == true) {
               //("Spec slice")
            ...
            const stepObj = this.client.startTestItem(Object.assign({
                type: (spec.fullName.indexOf('Allure') == -1 || spec.fullName.indexOf('Spec') !=-1) ? entityType.SUITE: entityType.STEP,
                description: (spec.fullName.indexOf('Allure') == -1) ? "duplicate" : spec.description,
                startTime: this.itemStartTime,
                name: (spec.fullName.indexOf('Allure') == -1) ? "." : spec.description,
            }, codeRef && { codeRef }), this.tempLaunchId, parent && parent.tempId);

            if(this.needToSlice == false) {
                this.setParentInfo({ tempId: stepObj.tempId, realID: stepObj.realId, startTime: this.itemStartTime, name: (spec.fullName.indexOf('Allure') == -1) ? "duplicate" : spec.description, type: "spec"});
            }
            else {
                //console.log("Splice")
                this.spliceParentInfo((this.middlePosition == -1) ? this.parentsInfo.length - 1 : this.middlePosition, { tempId: stepObj.tempId, realID: stepObj.realId, startTime: this.itemStartTime, name: spec.description, type: "spec"});
                this.needToSlice = false;
            }
    ...
    async specStartedType(spec, type) {
        const fullTestName = SpecificUtils.getFullTestName(spec);
        //console.log("FUlltestname: " + fullTestName);
        ...
        return this.currentPromise = promise.then((codeRef) => {
            ...
            if(this.needToSlice == false) {
                this.setParentInfo({ tempId: stepObj.tempId, startTime: this.itemStartTime, name: spec.description, type: type });
            }
            else {
                this.spliceParentInfo(this.parentsInfo.length - 1, { tempId: stepObj.tempId, startTime: this.itemStartTime, name: spec.description, type:type });
                this.needToSlice = false;
            }
    ...
    async specDone(spec) {
        ...
        if(spec.fullName.indexOf('Spec') != -1) {
            this.middleAlgorithm = false;
            this.middleTurn = this.middleTurn + 1;
        }
        ...
        */
        ...
        return this.currentPromise = promise.then((fileObj) => {
            ...
            let parent = this.getParentInfo();
            console.log(`delete SPEC SUITE  + ${parent.name}  ${parent.tempId} ${parent.type}`);
    ...
    async specDoneAttach(spec, screenshot) {
        // Для дебага
        // console.log("SpecDone:" + spec.description);
        let middleAlgo = this.middleAlgorithm;
        let position;
        ...
        */
        await this.currentPromise;
        if(spec.fullName.indexOf('Spec') != -1) {
            //console.log("middleAlgo OFF")
            middleAlgo = this.middleAlgorithm = false;
            this.middleTurn = this.middleTurn + 1;
        }
       ...
       //("screenshot:" + screenshot)
        ...
        return this.currentPromise = promise.then((fileObj) => {
            ...
            if(middleAlgo == true) {
                ...
                parent = this.parentsInfo[position];
                //console.log(`Position ${position} ${this.middlePosition}`)
                needToSlice = true;
                if(debugMode) {
                    deleteMode = 'Middle algo'
                }
            } else {
                parent = this.getParentInfo();
                deleteMode = 'Outside algo'
            }
            if(debugMode) {
                console.log(`${spec.fullName}: delete spec ${parent.name} by ${deleteMode}`);  
            }
            ...
            this.additionalCustomParams = {};
            // Обязательно передаем тип на удаление
            if(needToSlice == false) {
                this.finishParent(parent.name, "spec");
            }
            else {
                this.finishParentByPosition(position, "spec");
            }
            this.itemStartTime = null;
        });
    }
