    ...
    */
    async suiteStarted(suite) {
        ...
        */
        ...
        return this.currentPromise = promise.then((codeRef) => {
            ...
            if(this.parentsInfo.length > 0  && parent.type == "spec") {
            ...
            if(this.needToSlice == false) {
                ...
                this.spliceParentInfo(this.parentsInfo.length - 1, { tempId: suiteObj.tempId, startTime: this.itemStartTime, name: suite.description, type: 'suite' });
                this.needToSlice = false;
            }
            // для дебага
            /*
    ...
    async specStarted(spec) {
        ...
        return this.currentPromise = promise.then((codeRef) => {
            ...
            if(this.needToSlice == false) {
                this.setParentInfo({ tempId: stepObj.tempId, startTime: this.itemStartTime, name: spec.description, type: "spec" });
            }
            else {
                this.spliceParentInfo(this.parentsInfo.length - 1, { tempId: stepObj.tempId, startTime: this.itemStartTime, name: spec.description, type: "spec" });
                this.needToSlice = false;
            }
